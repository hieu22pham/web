...
export default function UserInfo() {
  ...
  const items = [
    ...
    },
    {
      ...
      children: [
        {
          label: <Link to="/admin/products">Tất cả sản phẩm</Link>,
          key: "/admin/products",
        },
        // <Button
        //   type='text'
        ...
        ...categories.map((item) => (
          item.category && {
            label:
              <Link to={`/admin/${item.category}`} onClick={() => handleCategoryClick(item)}>
                {item.category}
              </Link >,
            key: `/admin/${item.category}`,
          }
        ))
        , {
          label: <Button
      ...
      label: <Link to="/admin/slide-image">Hình ảnh slide</Link>,
      icon: <HomeOutlined />,
      key: "/admin",
    },
  ];
